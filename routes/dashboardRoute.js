                ...
                spotifyApi.spotifyApi.getMyCurrentPlaybackState().then(
                    function(data) {
                        if(data.body && data.body.is_playing){
                            ...
                            res.render('dashboard', {user:req.user, spotifyData:spotifyData, playing:playing, weather: weather, events: calenderData,map:mapData})
                    }, function(err) {
...
router.post('/call/make', (req,res) => {
    const number = process.env.TWILLIO_PHONE
       client.calls
    .create({
       twiml: '<Response><Say>Hello. This is test call from RSS motors.</Say></Response>',
       to: req.body.number,
       from: number
     })
...
router.post('/map/getloc', async (req,res)=> {
    ...
    let start_co=""
    let end_co=""
    const url_start = "https://geocode.search.hereapi.com/v1/geocode?q="+start+"&apiKey="+api_key
    ...
    await https.get(url_start, function(response)  {
        response.on("data", async function(data) {    
            ...
            const url_end = "https://geocode.search.hereapi.com/v1/geocode?q="+end+"&apiKey="+api_key
    console.log(url_end)
     https.get(url_end, function(response)  {
        response.on("data", function(data) {    
            ...
            console.log(end_co)
            url_nav="https://router.hereapi.com/v8/routes?transportMode=car&origin="+start_co+"&destination="+end_co+"&apiKey="+api_key+"&return=summary,actions,instructions,polyline"
            console.log(url_nav)
